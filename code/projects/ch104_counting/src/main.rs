        ...
        for w in t.history.windows(2) {
            ...
            if let Some(is_in) = crossing_dir(c.a, c.b, prev, curr) {
                ...
                let recent = last_fire
                    .map(|t0| ts.saturating_sub(t0) < c.dedup_ms)
                    .unwrap_or(false);
                if !recent {
                    last_fire = Some(ts);
                    tally.add(&t.label, is_in);
...
fn main() {
    ...
    println!(
        "\n—— 去重计数（按 track_id + {}ms 窗口，同一目标只数一次）——",
        c.dedup_ms
    );
    ...
    println!(
        "  person  进     {:>3}    {:>3}    {:>3}",
        np_in, dp_in, gt_person.0
    );
    println!(
        "  person  出     {:>3}    {:>3}    {:>3}",
        np_out, dp_out, gt_person.1
    );
    println!(
        "  car     进     {:>3}    {:>3}    {:>3}",
        nc_in, dc_in, gt_car.0
    );
    println!(
        "  car     出     {:>3}    {:>3}    {:>3}",
        nc_out, dc_out, gt_car.1
    );
    ...
    println!(
        ...
        dp_in,
        dp_out,
        dp_in as i32 - dp_out as i32
    );
    println!(
        "  car    净流入 = {} - {} = {}",
        dc_in,
        dc_out,
        dc_in as i32 - dc_out as i32
    );
...
mod tests {
    ...
    #[test]
    fn crossing_direction_excludes_the_infinite_extension() {
        let a = (0.0, 0.0);
        let b = (10.0, 0.0);
        assert_eq!(crossing_dir(a, b, (5.0, 1.0), (5.0, -1.0)), Some(true));
        assert_eq!(crossing_dir(a, b, (5.0, -1.0), (5.0, 1.0)), Some(false));
        assert_eq!(crossing_dir(a, b, (12.0, 1.0), (12.0, -1.0)), None);
    }

    #[test]
    fn dedup_suppresses_rapid_recrossing_per_track() {
        let counter = Counter {
            a: (0.0, 0.0),
            b: (10.0, 0.0),
            dedup_ms: 200,
        };
        let track = track_from_feet(
            7,
            "person",
            1.0,
            2.0,
        ...
        let (tally, events) = count_dedup(&counter, &[track]);
        assert_eq!(tally.get("person"), (1, 0));
