...
想象程序每帧都能识别目标，但摄像头拔掉后它无限等待，磁盘满后它直接退出，重启后又把旧事件推送一遍。算法可能正确，系统仍无法可靠使用。工程化就是把这些输入、资源和状态变化变成可定义、可观察、可恢复的行为。

可以先画一条最小链路：输入帧 → 检测结果 → 跟踪状态 → 事件 → 输出。输入帧要带时间戳和来源，事件要带唯一标识与模型/规则版本。发生错误时，才能回答“哪一路、哪一帧、哪个版本、在哪一步失败”。

例如输出端暂时不可用：先决定事件是否必须可靠送达，再决定是否落盘重试；重试应有退避和容量上限；重复投递使用稳定事件 ID，让接收端能够去重。这里没有一种脱离业务的万能方案。允许丢弃的实时预览帧与不能悄悄丢失的业务事件，可以采用不同策略。

**随堂核对**：函数失败时统一 `return Ok(())`，程序不崩溃了，算更可靠吗？不算。错误被隐藏后，调用者无法分辨“没有目标”和“根本没处理成功”。后面的错误类型、日志和测试，就是为了把这种区别留在接口里。
