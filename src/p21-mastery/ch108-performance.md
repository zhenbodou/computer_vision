...
## 为什么只把推理加速一倍，系统可能几乎没变快

假设一帧串行耗时为：解码 10 ms、预处理 5 ms、推理 20 ms、后处理 5 ms，总计 40 ms。把推理从 20 ms 优化到 10 ms，总计变成 30 ms，端到端加速比只有 `40/30≈1.33`，不是 2。

如果还存在排队，用户感受到的时延包括队列等待；若不同阶段并行流水作业，稳态吞吐又受最慢阶段制约，不能把各阶段耗时简单取倒数就当成总吞吐。先写清测量对象，再谈提升百分比。

报告 P95 时也要说明采样与计算方式。简单的最近秩法将 n 个耗时从小到大排，取第 `ceil(0.95n)` 个。只有 10 个请求时，这个估计几乎由最慢请求决定，很不稳定。应包含暖机、稳定负载和足够样本，保留超时与错误计数。

**随堂核对**：优化后只保留成功请求计算时延，超时变多但 P95 下降，可以说体验更好吗？不能。失败请求被排除会造成选择偏差，时延分布和失败率必须一起看。
