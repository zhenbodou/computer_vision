...
假设同一张红色图片在训练端识别正常，Rust 端却总是错误。先不要重新训练。我们沿管线找第一个发生差异的位置：

1. **解码图像**：宽高、方向和几个指定像素是否相同？相机或文件可能附带方向信息。
2. **输入张量**：RGB 还是 BGR？HWC 还是 CHW？值是 0–255、0–1，还是减均值除标准差后的结果？
3. **原始输出**：用完全相同的输入张量送入两个实现，比较模型输出。如果这里才开始不一致，再查算子、精度和后端。
4. **后处理**：如果原始输出相同但类别或框不同，检查类别表、激活函数、NMS 和坐标还原。

每次保存两端的同一中间量，再比较。只比最终画出来的图片，往往无法知道错在哪一层。对浮点值可用 `|a-b|≤atol+rtol×|b|` 判断，其中 atol 控制接近 0 的绝对误差，rtol 控制数值较大时的相对误差；容限应根据所用精度事先确定。

**随堂核对**：输入张量的 shape 完全一样，就能保证一致吗？不能。`[1,3,224,224]` 只描述容器尺寸，不说明三个通道的顺序，也不说明里面的数值是否缩放正确。先用纯红、纯绿、纯蓝的图核对通道，通常比盯着大模型日志更快。
