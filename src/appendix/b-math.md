...
**凸与非凸（convex / non-convex）**：凸函数可以想成没有坏的局部低谷的碗：若最小值存在，局部最小值也是全局最小值，但最优解不一定唯一。梯度下降是否收敛还取决于可微性、梯度性质和步长；例如对 `f(x)=x²` 使用学习率 2，会得到 `x_next=-3x`，越走越远。严格凸函数若存在最优解则解唯一；非凸像连绵起伏的山谷，有很多「局部最低点」，可能卡在半山腰。深度网络的损失几乎都是**非凸**的——好在实践中找到的局部最优通常「够好用」。
...
**刚体 / 相似变换**：刚体变换只有平移和旋转，保持距离；相似变换再允许统一的等比缩放，保持角度与长度比例，但通常不保持距离。2D 旋转矩阵是：
...
**叉积（cross product）**：在 2D 里它退化成一个标量 \\( \mathbf{a} \times \mathbf{b} = a_x b_y - a_y b_x \\)。它的**符号定方向**：在 x 向右、y 向上的坐标系中，正说明 b 在 a 的逆时针一侧，负则顺时针，零则共线。图像通常 y 向下，此时屏幕上看到的顺逆时针方向相反；写绊线规则前先画两个点核对。这正是**绊线检测判断「从哪个方向穿过」**（[第 87 章](../p18-events/ch87-tripwire.md)）、以及判断点在多边形 ROI 内外（[第 84 章](../p18-events/ch84-roi-filter.md)）的核心技巧。
