...
## 第 20 部分阶段验收：把演示升级为独立项目

**任务**：从第 97–107 章选一个项目，完成附录 J 全部关卡；先画出现有模拟模块与待接入真实模块。

**提交与核对**：真实数据清单、模型契约、基线/消融、逐样本结果、故障日志与运行手册。

**失败实验**：由另一人或独立保留批次验收；运行合成 Demo 不算通过真实项目。

先完成 [附录 I 的分级实训](../appendix/i-guided-labs.md)，再按 [附录 G](../appendix/g-expert-standard.md) 记录证据。出现不达标项时保留失败报告、提出下一轮实验，不事后修改验收口径。
