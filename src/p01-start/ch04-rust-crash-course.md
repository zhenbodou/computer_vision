...
## 读一段 Rust 时，先分清“谁拥有，谁借用”

下面是一个完整小程序，可以临时替换第 3 章工程的 `src/main.rs`，运行后再还原原来的图像程序。它不需要第三方库。

```rust
fn brighten(pixels: &mut [u8], delta: u8) {
    for pixel in pixels {
...
fn main() {
    let mut pixels = vec![10_u8, 100, 250];
    let before = pixels.clone();
    brighten(&mut pixels, 20);
    println!("修改前：{before:?}");
    println!("修改后：{pixels:?}");
}
```

`pixels` 拥有这一块像素数据；`&mut pixels` 暂时把“允许修改”的权限借给函数，不把整块图像复制过去。参数 `&mut [u8]` 表示可修改的像素切片，不要求调用方一定使用 Vec，也可以接收数组的切片。

循环里的 `pixel` 是指向某个像素的可变引用；赋值左侧的 `*pixel` 表示修改它指向的数。`saturating_add` 是饱和加法，250 加 20 后限制在 255。返回后，借用结束，main 可以继续使用 pixels。

输出应为修改前 `[10,100,250]`、修改后 `[30,120,255]`。这里 `clone()` 是为了教学比较，明确复制了数据；真实大图不应为了躲开借用错误到处 clone。先判断处理函数究竟需要只读借用、可写借用，还是取得所有权。

**随堂核对**：把函数参数改成 `&[u8]`，还允许给 `*pixel` 赋值吗？不允许，因为它只借到了读取权限。编译器是在保护这个约定，而不是“故意让代码难写”。
