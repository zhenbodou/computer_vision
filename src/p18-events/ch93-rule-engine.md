...
## 顺序、状态与配置版本，决定规则真正的含义

假设先做类别过滤，再做 ROI，再做 3/5 时间投票，最后做 cooldown。这表示只有通过前面条件的观察才参与确认。但如果 ROI 过滤后直接丢掉整帧，时间投票模块可能根本收不到“这一帧没有命中”的证据，旧命中状态就不会按预期消退。

因此数据接口通常要区分“有效帧但目标列表为空”与“帧没有到达/处理失败”。过滤器删除检测结果，不一定应删除时间推进本身。规则图里每个阶段应写明输入、输出、是否持有状态以及失败时怎样继续。

配置热更新也不只是换一个 JSON。将 ROI 改到新位置或把投票阈值从 3/5 改为 4/5 时，旧窗口是否仍适用？可以选择重置相关状态，或实现明确的状态迁移；无论选哪种，都应为之后的事件记录配置版本，方便回放解释。

**随堂核对**：同一批输入重放两次却得到不同告警，先查什么？先查是否混入系统当前时间、无种子的随机操作、未清理的历史状态或不同配置版本。可重复回放是定位规则问题的基础。
