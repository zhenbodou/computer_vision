...
## 超时以后重试，为什么可能收到两条相同告警

假设接收端已经把事件保存成功，但返回确认的网络包丢了。发送端只看到超时，不知道“没有收到”和“收到但确认丢失”是哪一种；直接重试可能再次送达同一事件。

给每个业务事件分配稳定 event_id，重试沿用同一个 ID，接收端按 ID 做幂等处理。幂等的意思是同一事件处理多次，最终业务效果与处理一次相同。每次重试都重新生成 ID，会破坏这个机制。

还要区分事件去重与传输去重：cooldown 决定哪些业务事件值得生成；幂等处理决定同一已生成事件的多次投递是否重复生效。用长 cooldown 掩盖网络重复投递，会连真正的新事件也一起压掉。

**随堂核对**：日志写“开始发送”就算成功送达吗？不算。至少区分已生成、入队、发送尝试、对方确认、永久失败等状态；必要时把待发事件持久化，并记录有上限的重试策略。多出口分发还要分别跟踪成功状态，一个出口成功不能覆盖另一个的失败。
