...
先看一行 `[10,10,10,10]`。它没有起伏，只有恒定亮度；离散傅里叶变换中，零频率分量等于四个数的和 40，其余频率为 0。这一项常叫 **DC 分量**，记录整段信号的平均亮度乘以样本数。

再看 `[10,-10,10,-10]`，它的均值为 0，却每个位置都在正负交替，变化非常快，能量集中在最高交替频率。真实灰度不能为负，但图像减去均值后的信号完全可以为负，傅里叶分析不受影响。

频域数据通常是复数，可以理解为同时记录一种波的强弱与相位。幅度谱只显示强弱；相位描述这些波怎样对齐。漂亮的幅度谱图片只是用于观察，不能把它直接当成完整频谱做逆变换，否则丢了相位就丢了结构信息。

**随堂核对**：低通滤波后细小文字变糊，是否一定是 bug？不是。笔画边缘本身包含高频变化，滤去高频时噪声和有效细节可能一起丢失。频域位置并不自带“噪声”标签，需要结合噪声模式选择滤波方式。
