...
## 两个像素就能看出：reshape 不等于换布局

假设图片只有一行两列：左边纯红 `(255,0,0)`，右边纯绿 `(0,255,0)`。HWC 的连续数据按“一个像素的 RGB，再下一个像素的 RGB”排列：

```text
HWC，shape=[1,2,3]：255, 0, 0,  0, 255, 0
CHW，shape=[3,1,2]：255, 0,     0, 255,     0, 0
                    红平面     绿平面       蓝平面
```

直接 `reshape` 只是重新解释维度，并不会把元素搬到新的通道顺序中。换布局需要交换轴，或者像下面示例一样明确按 `(c,y,x)` 写入；交换轴得到的视图还可能不是连续内存，下游要求连续时再生成连续副本。

如果输入规范是除以 255，则 CHW 浮点结果为 `[1,0,0,1,0,0]`。若还要做 `(x-mean)/std`，必须在正确的通道上使用对应的 mean/std。模型要求什么就做什么，不能看到“归一化有用”便重复做两次。

**随堂核对**：给这张图加 batch 维，NCHW 的形状是什么？是 `[1,3,1,2]`；元素数量仍然是 6。N 是图片张数，C 是每张图的通道数，别因为二者都可能等于 3 就把它们混用。调试真实模型前先跑这个双色小例子，能迅速发现通道和布局问题。
