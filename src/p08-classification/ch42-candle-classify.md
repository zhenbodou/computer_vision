...
> ⚠️ **注意（本章的诚实边界）**：真正的 ResNet / MobileNet / ViT 是几十上百 MB 的权重，要联网从 Hugging Face 下载。为了让你**此刻就能跑通、且完全可复现**，本章改用一个**我们自己训练的小 CNN + 代码合成的小图**（延续 [第 40 章](../p07-dl-basics/ch40-burn-train.md) 的三类形状：竖条 / 横条 / 方块）。**本实验只证明权重保存、加载和推理闭环。** 外部 safetensors 文件仅保存张量，不自动提供网络结构；还需逐项核对层名、形状、数据类型、模型配置、预处理和类别表。真实模型可能需要这里没有实现的算子。完成本章后，再按 [附录 J](../appendix/j-capstone.md) 的模型契约关卡接入外部模型。
...
设模型有“方块、横条、竖条”三个类别，输出 logits 为 `[2,1,0]`。logit 是未经归一化的分数，可以为负，也不要求总和为 1。对三个数做 softmax，得到约 `[0.665,0.245,0.090]`，选最大值对应的类别“方块”。类别表的顺序必须与训练时一致，否则计算全对也会显示错误名字。

为了避免指数溢出，可先减最大值，再算 softmax：`[2,1,0]→[0,-1,-2]`。所有元素减去同一个数不会改变最终概率比例。不要给要求 logits 的交叉熵再预先套一次 softmax，也不要因为输出恰好在 0–1 内就猜测模型已经归一化；应以模型定义为准。

还要区分“这三个候选中最像方块”与“确定是方块”。输入一张从没见过的照片，模型也会从这三个类别里选一个，甚至给出高分。softmax 高分本身不证明输入属于已知类别，更不是经过验证的正确率保证。

**随堂核对**：把 logits 同时加上 1000，类别应否改变？不应，稳定 softmax 的概率也不变。若程序得到 NaN，先检查数值实现，而不是怀疑图片标签。
...
三张图全部识别正确，概率都在 0.9 以上，而且 softmax 分布加起来正好是 1。**这条从"一张图"到"一个类别名 + 置信度"的完整链路，就是所有图像分类推理的通用骨架**——换成真实 ResNet 时，仍需核对权重命名、网络实现、输入缩放与裁剪、归一化以及输出类别映射，并用参考输出验证，不能仅凭文件成功加载判断正确。
...
- 诚实提醒：本章用小模型 + 合成图是为了**免下载、即刻可跑**；掌握的是权重加载的基本机制；真实模型的结构、算子与输入输出契约需要额外对齐。
