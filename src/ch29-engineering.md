...
/// 最终检测结果
struct Done   { id: u64, dets: Vec<Detection> }
```
...
**下一步学什么？** 几个方向供参考：往**训练侧**走，学会自己标注数据、微调模型（本书专注推理侧，训练是另一片天地）；往**更极致的性能**走，深入 TensorRT/量化/算子级优化；往**更大的系统**走，多机多路调度、消息队列、云边协同；或者干脆挑一个真实场景（你家的猫、小区的车、产线的瑕疵），把本书的链路端到端跑通一遍——**纸上得来终觉浅，绝知此事要躬行**。附录 A~D 是你随时可查的数学、crate、术语和踩坑手册。

祝你在机器视觉的工程路上，看得懂图像，也看得懂模型。
