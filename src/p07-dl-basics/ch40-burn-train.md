# 第 40 章 用 burn 从零训练一个 CNN：合成形状入门与 MNIST 迁移
...
先把几个经常一起出现的词拆开。**样本（sample）**是一张图和它的标签；**批次（batch）**是一次送进网络的一小组样本；**轮次（epoch）**是训练数据被完整遍历一次。假设有 100 张训练图，batch size 为 20，每轮通常有 5 次参数更新。若最后一批不足 20，保留还是丢弃要明确记录。

一次更新的顺序是：取出图像与标签 → 前向算预测 → 损失比较预测和标签 → 反向求参数梯度 → 优化器修改参数。训练集里的标签参与这条更新链；验证集只用于评价和选择配置，不应调用优化器更新。

**第一次运行的排错方法**：先只拿 8 张训练图，关闭随机增强，让模型反复学习它们。这个小集合仍学不会时，优先检查标签是否错位、像素是否缩放、loss 是否使用正确的输出格式、参数是否真的更新；此时换大数据集通常只会让问题更难找。小集合拟合成功也不证明泛化好，它只是排除最基础的训练链路错误。

本章代码实际训练的是合成形状分类。它能教会训练循环，但没有完成真实 MNIST 手写数字实验。两者都属于图像分类，数据的类别数、尺寸与困难程度不同，学习效果不能相互代称。把本章流程学会后，再以独立划分的真实数字数据检验泛化。
