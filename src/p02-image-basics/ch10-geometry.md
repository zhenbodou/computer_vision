...
想把宽 2 的一行图片放大到宽 4，原像素值为 `[0,100]`。如果简单把原坐标乘 2，两个像素只能写到目标位置 0 和 2，位置 1 和 3 谁来填？这就是正向搬运可能产生空洞的原因。

常见做法是遍历每个输出位置，通过逆变换找它在输入中的位置，再插值得到颜色。若采用像素中心对齐，源坐标为 `(x_out+0.5)/2-0.5`，四个输出中心对应 `[-0.25,0.25,0.75,1.25]`。对边界采取复制、内部使用线性插值，得到 `[0,25,75,100]`。若使用其他坐标对齐约定，数值可能不同，所以要比较两套预处理时，插值和对齐方式都必须记录。

对模型输入还常见**等比缩放加填充（letterbox）**：640×480 缩放到 320×320，实际图像先变成 320×240，上下各填 40。模型输出框 `[50,70,150,170]`，先从 y 减 40，再把所有坐标除以 0.5，回到原图就是 `[100,60,300,260]`。

**随堂核对**：直接将原图拉伸到 320×320，再沿用上述还原公式，对吗？不对。拉伸的 x、y 比例不同，也没有上下填充。几何变换与逆变换要成对保存，不能看最终尺寸相同就当成同一种预处理。
