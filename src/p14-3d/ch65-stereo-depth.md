...
设校正后双目的焦距是 `f=800 px`，两相机基线是 `B=0.1 m`，同一点在左右图的横坐标相差 `d=40 px`。深度 `Z=fB/d=2 m`。像素单位在分子分母相消，结果跟随基线使用米；若基线误填成 100，却仍把结果解释成米，就会产生一千倍错误。

视差误差对深度的局部影响约为 `|ΔZ|≈fB/d²×|Δd|`。若视差误差为 1 px，40 px 视差处的深度误差约为 0.05 m；只有 10 px 视差时，深度为 8 m，同样的 1 px 误差对应约 0.8 m。远处难测准不是简单换个输出精度格式就能解决的。

当视差为 0 或无有效匹配时，应输出无效深度/掩码，而不是把距离写成 0 米。纹理重复、反光和遮挡可能让匹配到的像素根本不是同一个点，此时误差传播公式的“小误差”前提已经失效，需要先拒绝错误匹配。

**随堂核对**：保持视差误差与焦距不变，增大基线是否总是没有代价？不是。它能提高某些距离的深度分辨能力，但也可能增加遮挡、降低视野重叠并使匹配更困难，必须结合工作距离设计。
