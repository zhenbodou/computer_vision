...
## 为什么合成标定误差接近零，还不能拿去测量

本章合成数据先用已知相机模型投影出角点，再用匹配的模型反求参数。没有真实角点噪声、镜头失配和板材误差时，重投影残差接近数值精度是合理的，它证明的是算法在理想条件下自洽。

真实标定的输入则是照片里检测出的角点。模糊会使角点漂移，棋盘打印尺寸和板面平整度也会改变真值。需要不同倾角、不同图像位置和适当距离的多张图，而不是固定姿态连拍许多张近重复照片。视角变化是在给未知参数提供不同约束。

**重投影误差**回答的是“估计参数能否把棋盘点投回观察位置”，不是“任意物体的毫米测量一定准确”。应另外保留未参与拟合的图像，检查角点误差随位置的分布，并用有已知尺寸的对象做物理尺度核对。

**随堂核对**：棋盘格实际 20 mm，你却填成 25 mm，拟合的像素重投影误差一定会显著增大吗？不一定；外参平移尺度可以随之改变，图像投影仍然吻合，但恢复的平移和物理测量尺度会错误。这个例子说明只盯一个残差可能遗漏什么。
