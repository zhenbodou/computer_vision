...
## 先问数据够不够决定答案，再问用哪个求解器

两张单目图像可以提供相对几何，但通常不能独立确定真实尺度。设空间点与两相机间平移都放大两倍，归一化投影中的比例仍然相同，图像可以不变。因此由本质矩阵恢复的平移通常只有方向，不能直接当成“移动了多少米”。需要已知尺寸、基线或其他传感器提供尺度。

纯旋转时，相机没有提供有效平移基线，三角化深度缺少约束；观察方向非常接近时，深度也会对像素误差极敏感。这样的失败属于几何可观测性问题，不是把迭代次数调大就必然能解决。

PnP 的信息不同：它已有带坐标的三维点，再对应二维像素去求相机姿态。三维点的单位决定平移单位。点过于集中、近共线或观测配置退化时，解仍可能不稳，需要检查残差、正深度、空间覆盖与多解，而不是只取求解器返回的第一组数。

**随堂核对**：重投影误差低就能保证姿态唯一吗？不能。弱约束或存在多解时，不同参数可能在当前观测上都拟合得很好。额外视角和先验有时比换优化器更重要。
