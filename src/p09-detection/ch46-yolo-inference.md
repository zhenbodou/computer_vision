...
沿用本章 48×48 输入、6×6 网格、stride=8 的约定。取第 i=2 行、第 j=3 列，设经 sigmoid 后格内偏移为 `(0.25,0.5)`，宽高比例为 `(0.25,0.5)`。

中心为 `((3+0.25)×8,(2+0.5)×8)=(26,20)`；宽高为 `(0.25×48,0.5×48)=(12,24)`。把中心各减/加半个宽高，就得到 `xyxy=[20,8,32,32]`。

若 objectness 为 0.8，该类别分数为 0.75，本章相乘策略得到 score=0.6。置信阈值为 0.5 时保留，再进入同类 NMS。这里是先对原始输出应用各自的激活函数，然后解码，不能把已经 sigmoid 的 0.25 再 sigmoid 一次。

**这套七个数只适用于本章检测头。** 真实 YOLO 导出可能已经解码、输出维度次序不同、没有单独 objectness，或者已经包含 NMS。读 `[1,7,6,6]` 的下标代码不能直接搬到未知输出上。先打印每个输出的名称、shape 和数值范围，再核对导出模型的定义。

**随堂核对**：同一格子里出现两个中心很近的目标，本章每格仅一个预测能同时表达两者吗？不能可靠地表达，这正是教学模型的容量限制之一。降低阈值无法修复输出表示本身的限制，真实检测器需要更丰富的候选与分配机制。
