...
假设标定比例为 `0.02 mm/px`，两条边相距 120 px，长度就是 2.4 mm。如果算法把边的位置估计为 120.1 px，结果为 2.402 mm；这只表明模型允许连续位置，不自动证明真实精度达到 0.002 mm。

若比例实际上是 0.021 mm/px，同一个 120 px 对应 2.52 mm，系统偏差已有 0.12 mm，远大于多保留几个小数的收益。再精细的边缘拟合也不能弥补错误的物理尺度。

**重复性**指同一条件下反复测量是否接近；**偏差**指平均测量值离参考值有多远。一个每次都报 2.400 mm、真实值却为 2.520 mm 的系统，重复性很好，偏差仍很大。实际报告至少把平均误差、重复测量标准差和标定条件分开写。

**随堂核对**：把计算类型从 f32 换成 f64 能消除上面的 0.12 mm 偏差吗？不能。那是尺度模型错误，不是浮点舍入误差。真正的排查顺序要包含成像、边缘定义、标定和温度/姿态变化，最后才是数值精度。
...
1. 把像素尺寸设为 0.02 mm/px，手算 120 px 的长度（2.4 mm）；然后把尺度错设为 0.021，核对系统偏差为 0.12 mm。
2. 固定同一条合成边，分别加入不同噪声，重复测量 30 次，报告均值与标准差；说明重复性好为什么不等于绝对准确。
3. 改变边缘对比度和拟合窗口，画误差随参数的变化；再做下文专家考核。
